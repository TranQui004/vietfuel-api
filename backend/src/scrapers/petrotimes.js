...
import { parsePrice, deduplicate } from './utils.js';
...
async function scrapePetrotimes() {
  ...
  console.log(`[Scraper:Petrotimes] Xong. ${parsedObj.length} items. (${((Date.now() - start) / 1000).toFixed(2)}s)`);
  return { prices: parsedObj, scrapedAt: new Date().toISOString(), source: 'https://petrotimesgroup.com', priceDate: null };
}

export {  scrapePetrotimes  };
